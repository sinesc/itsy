...
use crate::prelude::*;
use crate::shared::{BindingContainer, {typed_ids::{BindingId, FunctionId, ScopeId, TypeId, ConstantId}, numeric::Numeric, Progress, parts_to_path}};
...
/// A named value. This can either be a constant or a variable.
pub enum Value {
    Unknown { position: Position, ident: Ident },
    Constant(Constant),
    Variable(Variable),
}
...
impl Typeable for Value {
    fn type_id(self: &Self, bindings: &impl BindingContainer) -> Option<TypeId> {
        impl_matchall!(self, Value, item, { item.type_id(bindings) }, _ => None)
    }
    fn type_id_mut<'t>(self: &'t mut Self, bindings: &'t mut impl BindingContainer) -> &'t mut Option<TypeId> {
        impl_matchall!(self, Value, item, { item.type_id_mut(bindings) }, _ => { panic!("Attempted to set type of Value::Unknown") })
    }
}

impl Resolvable for Value {
    fn is_resolved(self: &Self) -> bool {
        impl_matchall!(self, Value, item, { item.is_resolved() }, _ => false)
    }
    fn num_resolved(self: &Self) -> Progress {
...
impl Positioned for Value {
    fn position(self: &Self) -> Position {
        impl_matchall!(self, Value, item, { item.position() }, Value::Unknown { position, .. } => { *position })
    }
}

impl Debug for Value {
    fn fmt(self: &Self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        impl_matchall!(self, Value, item, { write!(f, "{:#?}", item) }, Value::Unknown { position, ident } => { write!(f, "Value::Unknown {{ position: {position:?}, ident: {ident:?}") })
    }
}

impl Display for Value {
    fn fmt(self: &Self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        impl_matchall!(self, Value, item, { write!(f, "{}", item) }, _ => { write!(f, "Value::Unknown") })
    }
}
