...
impl ParsedProgram {
    ...
    pub fn set_link_state(self: &mut Self, link_state: LinkState) {
        let LinkState { scope_parent_map, bindings: binding_map, .. } = link_state;
...
pub struct LinkState {
    ...
    pub(super) bindings: Set<BindingId>,
}
...
impl<'a> Input<'a> {
    ...
    pub fn pop_scope(self: &Self, revert: Option<(ScopeId, BindingId)>) {
        ...
        if let Some((revert_scope_id, revert_binding_id)) = revert {
            ...
            state.link_state.bindings.retain(|&binding_id| binding_id < revert_binding_id);
        } else {
            ...
            for binding_id in scope_bindings.into_iter() {
                state.link_state.bindings.insert(binding_id);
            }
        }
    }
