...
pub struct ParsedProgram {
    ...
    pub(crate) binding_ids: Set<BindingId>,
}

impl ParsedProgram {
    /// Returns an empty ParsedProgram instance.
    pub fn new() -> Self {
        Self { modules: Vec::new(), scope_parent_map: UnorderedMap::new(), binding_ids: Set::new() }
    }
    ...
    pub fn set_link_state(self: &mut Self, link_state: LinkState) {
        let LinkState { scope_parent_map, binding_ids, .. } = link_state;
...
pub struct LinkState {
    ...
    pub(super) binding_ids: Set<BindingId>,
}
...
pub(super) struct Scope {
    ...
    kind: ScopeKind,
    binding_lookup: Map<String, BindingId>,
    have_bindings: Set<BindingId>,
...
impl<'a> Input<'a> {
    ...
    pub fn push_scope(self: &Self, scope_type: ScopeKind) -> (ScopeId, BindingId) {
        ...
        state.scope_stack.push(Scope { scope_id, parent_scope_id, kind: scope_type, binding_lookup: Map::new(), have_bindings: Set::new(), require_bindings: Set::new() });
    ...
    pub fn pop_scope(self: &Self, revert: Option<(ScopeId, BindingId)>) {
        ...
        if let Some((revert_scope_id, revert_binding_id)) = revert {
            ...
            state.link_state.binding_ids.retain(|&binding_id| binding_id < revert_binding_id);
        } else {
            state.link_state.scope_parent_map.insert(scope.scope_id, scope.parent_scope_id);
            let scope_bindings = replace(&mut scope.have_bindings, Set::new());
            for binding_id in scope_bindings.into_iter() {
                state.link_state.binding_ids.insert(binding_id);
            }
        }
    }
    ...
    pub fn add_binding(self: &Self, name: &str) -> BindingId {
        ...
        // if binding already exists we have to create a new binding id for it, but we also need to update the id a binding lookup points to
        state.scope_stack.last_mut().unwrap().binding_lookup.insert(name.to_string(), binding_id);
        state.scope_stack.last_mut().unwrap().have_bindings.insert(binding_id);
    ...
    pub fn has_binding(self: &Self, name: &str) -> ScopeBindingType {
        ...
        for scope in state.scope_stack.iter().rev() {
            if let Some(&binding_id) = scope.binding_lookup.get(name) {
    ...
    pub fn require_binding(self: &Self, binding_id: BindingId) {
        ...
        for scope in state.scope_stack.iter_mut().rev() {
            if scope.have_bindings.contains(&binding_id) {
